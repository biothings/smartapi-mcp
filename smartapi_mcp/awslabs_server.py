...
async def create_mcp_server(config: Config) -> FastMCP:
    ...
    try:
        ...
        openapi_spec = load_openapi_spec(
            url=config.api_spec_url, path=config.api_spec_path
        )
        ...
        from awslabs.openapi_mcp_server.auth import (
            get_auth_provider,
        ...
        if config.auth_type and config.auth_type != "none":
            logger.debug(
                f"Registering authentication provider for type: {config.auth_type}"
            )
        ...
        def handle_auth_error(auth_type, error_message):
            ...
            logger.error(
                "Server shutting down due to authentication configuration error."
            )
            sys.exit(1)
        ...
        if openapi_spec and isinstance(openapi_spec, dict) and "info" in openapi_spec:
            if "title" in openapi_spec["info"] and openapi_spec["info"]["title"]:
                ...
                logger.info(
                    f"Updated API name from OpenAPI spec title: {config.api_name}"
                )
        ...
        try:
            ...
            prompt_manager.register_api_resource_handler(
                server, config.api_name, client
            )
        ...
        async def health_check() -> dict[str, Any]:
            ...
            try:
                ...
                api_health = False
                api_message = f"Error connecting to API: {e!s}"
