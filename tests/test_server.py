...
from smartapi_mcp import get_mcp_server, merge_mcp_servers, get_merged_mcp_server
...
async def test_get_merged_mcp_server():
    """Test merge_mcp_servers helper function."""
    merged_server = await get_merged_mcp_server(smartapi_ids = [test_api_id_1, test_api_id_2])
    assert isinstance(merged_server, FastMCP)
    ...
    merged_server = await get_merged_mcp_server(
        smartapi_q = f"_id: {test_api_id_1}"
    )
    assert isinstance(merged_server, FastMCP)
    tools = await merged_server.get_tools()
    assert len(tools) >= 4
